...
#DOCKER_CONTENT_TRUST=0 docker build .. -f Dockerfile -t ffmpeg-windows-build # builds an image
...
if [ $? -eq 0 ]; then
    ...
    docker run --name ffmpegbuilder -it -v /data/public/ffmpeg-windows-build-helpers:/ffmpeg-windows-build-helpers ffmpeg-windows-build || docker start ffmpegbuilder -i
